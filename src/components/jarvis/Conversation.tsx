...
const quick = [
  ...
  "List active agents",
  "Check system battery and thermals",
  "Help",
];

export function Conversation() {
  const { messages, sendMessage, clearChat, thinking, selectedPersona } = useJarvis();
    ...
    <section className="bezel flex min-h-0 flex-col overflow-hidden rounded-2xl">
      <div className="flex items-center justify-between border-b border-[oklch(0_0_0/35%)] px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="neu-inset px-2 py-0.5 rounded-full text-[9.5px] font-mono text-cyan-hud font-bold">
      ...
      <div className="screen flex max-h-64 min-h-32 flex-1 flex-col gap-4 overflow-y-auto px-4 py-4">
        {messages.length === 0 && (
          <p className="py-8 text-center text-xs text-muted-foreground">
            Channel clear. Type a directive or speak via live voice.
          </p>
        )}
        {messages.map((m) => (
          <div key={m.id} className="animate-rise-in relative flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex items-baseline gap-2">
                ...
                >
                  {m.role === "jarvis" ? (m.personaId ? m.personaId.toUpperCase() : "JARVIS") : "YOU"}
                </span>
      ...
      <div className="flex flex-wrap gap-2 px-4 pb-2 pt-3">
        {quick.map((q) => (
          <button
            key={q}
            onClick={() => submit(q)}
            className="key rounded-full px-3 py-1 text-[11px] font-semibold text-muted-foreground hover:text-cyan-hud cursor-pointer"
          >
            {q}
          </button>
        ))}
      ...
      >
        <div className="neu-inset flex min-w-0 flex-1 items-center rounded-xl px-3.5 py-2.5">
          <input
            ...
            placeholder={`Directive for ${selectedPersona.name}…`}
        ...
        <button
          ...
          className="key grid h-11 w-11 shrink-0 place-items-center rounded-xl text-cyan-hud glow-ring cursor-pointer"
        >
