import { useState } from "react";
import { useJarvis, useStats } from "../JarvisProvider";
import { cn } from "@/lib/utils";
import { Send, Shield, Sparkles, Volume2 } from "lucide-react";
import { toast } from "sonner";

export function AgentsView() {
  const {
    agents,
    toggleAgent,
    setAgentStatus,
    selectedPersona,
    handleSwapPersona,
    handleDelegateTask,
    mutedRelayEvents,
  } = useJarvis();
  const stats = useStats();

  const [delegateOpenFor, setDelegateOpenFor] = useState<string | null>(null);
  const [delegateTaskText, setDelegateTaskText] = useState("");

  const handleSendDelegate = (agentId: string) => {
    if (!delegateTaskText.trim()) return;
    handleDelegateTask(delegateTaskText.trim(), agentId);
    toast.success(`Task delegated to ${agentId.toUpperCase()}`);
    setDelegateTaskText("");
    setDelegateOpenFor(null);
  };

  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="font-display etched text-2xl font-bold tracking-wide">AI Agent Swarm</h1>
            <span className="neu-inset px-2.5 py-0.5 rounded-full text-[10.5px] font-bold text-cyan-hud border border-cyan-500/20">
              Orchestrator v4.0
            </span>
          </div>
          <div className="mt-2 flex gap-5 text-xs text-muted-foreground">
            <span className="flex items-center gap-2">
              <i className="led h-2 w-2 bg-emerald-hud text-emerald-hud" />
              <b className="text-foreground">{stats.running}</b> running
            </span>
            <span className="flex items-center gap-2">
              <i className="h-2 w-2 rounded-full bg-muted-foreground/60" />
              <b className="text-foreground">{stats.stopped}</b> suspended
            </span>
            {mutedRelayEvents.length > 0 && (
              <span className="flex items-center gap-1.5 text-indigo-400">
                <Shield className="w-3.5 h-3.5" />
                <b>{mutedRelayEvents.length}</b> background relays
              </span>
            )}
          </div>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => agents.forEach((a) => setAgentStatus(a.id, "running"))}
            className="key rounded-xl px-3.5 py-2 text-[12px] font-bold text-emerald-hud cursor-pointer"
          >
            Activate all
          </button>
          <button
            onClick={() => agents.forEach((a) => setAgentStatus(a.id, "stopped"))}
            className="key rounded-xl px-3.5 py-2 text-[12px] font-bold text-amber-hud cursor-pointer"
          >
            Suspend all
          </button>
        </div>
      </header>

      <div className="grid min-h-0 flex-1 auto-rows-min grid-cols-[repeat(auto-fill,minmax(18rem,1fr))] gap-3.5 overflow-y-auto pb-4 pr-1">
        {agents.map((a) => {
          const isSelected = selectedPersona.id === a.id;
          const isDelegateOpen = delegateOpenFor === a.id;

          return (
            <article
              key={a.id}
              style={
                isSelected
                  ? {
                      borderColor: `color-mix(in oklab, ${a.accent} 50%, transparent)`,
                      boxShadow: `0 0 20px color-mix(in oklab, ${a.accent} 20%, transparent)`,
                    }
                  : undefined
              }
              className={cn(
                "neu gloss animate-rise-in flex flex-col gap-2.5 rounded-2xl p-4 transition-all hover:-translate-y-0.5 relative",
                isSelected && "border",
              )}
            >
              {isSelected && (
                <span
                  style={{
                    backgroundColor: `color-mix(in oklab, ${a.accent} 20%, transparent)`,
                    color: a.accent,
                    borderColor: `color-mix(in oklab, ${a.accent} 40%, transparent)`,
                  }}
                  className="absolute top-2 right-2 px-2 py-0.5 rounded-full text-[9px] font-bold tracking-wider border"
                >
                  ACTIVE VOICE
                </span>
              )}

              <div className="flex items-start justify-between gap-2">
                <span
                  className="neu-inset grid h-11 w-11 shrink-0 place-items-center rounded-xl text-lg"
                  style={{ color: a.accent }}
                >
                  {a.icon}
                </span>
                <span
                  className={cn(
                    "neu-inset flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-extrabold tracking-wider",
                    a.status === "running" ? "text-emerald-hud" : "text-muted-foreground",
                  )}
                >
                  <i
                    className={cn(
                      "h-1.5 w-1.5 rounded-full",
                      a.status === "running" ? "led bg-emerald-hud text-emerald-hud" : "bg-muted-foreground/60",
                    )}
                  />
                  {a.status === "running" ? "ONLINE" : "OFFLINE"}
                </span>
              </div>

              <div>
                <h3 className="text-sm font-bold flex items-center gap-2">
                  {a.name}
                  {a.voiceName && (
                    <span className="font-mono text-[10px] text-muted-foreground font-normal">
                      (Voice: {a.voiceName})
                    </span>
                  )}
                </h3>
                <p className="text-[11.5px] leading-relaxed text-muted-foreground mt-1">{a.desc}</p>
              </div>

              <div className="mt-auto space-y-2.5 pt-2">
                <div className="flex items-center justify-between font-mono text-[10.5px] text-muted-foreground">
                  <span>{a.tasks} operations</span>
                  <span>{a.status === "running" ? (a.uptimeMin >= 60 ? `${Math.floor(a.uptimeMin / 60)}h ${a.uptimeMin % 60}m uptime` : `${a.uptimeMin}m uptime`) : "suspended"}</span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-[oklch(0.13_0.01_256)] shadow-[inset_0_1px_3px_oklch(0_0_0/75%)]">
                  <i
                    className="block h-full rounded-full transition-all duration-700"
                    style={{
                      width: `${a.load}%`,
                      background: `linear-gradient(90deg, ${a.accent}, color-mix(in oklab, ${a.accent} 45%, transparent))`,
                      boxShadow: `0 0 8px ${a.accent}`,
                    }}
                  />
                </div>

                {isDelegateOpen ? (
                  <div className="space-y-2 pt-1 border-t border-white/5 animate-fade-in">
                    <input
                      type="text"
                      placeholder={`Task for ${a.name}…`}
                      value={delegateTaskText}
                      onChange={(e) => setDelegateTaskText(e.target.value)}
                      onKeyDown={(e) => e.key === "Enter" && handleSendDelegate(a.id)}
                      className="w-full text-xs neu-inset rounded-xl px-3 py-2 text-foreground placeholder:text-muted-foreground outline-none"
                    />
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => handleSendDelegate(a.id)}
                        className="flex-1 key rounded-lg py-1.5 text-xs font-bold text-cyan-hud"
                      >
                        Send
                      </button>
                      <button
                        onClick={() => setDelegateOpenFor(null)}
                        className="px-3 key rounded-lg py-1.5 text-xs font-bold text-muted-foreground"
                      >
                        Cancel
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-1.5 pt-1">
                    <button
                      onClick={() => handleSwapPersona(a.id)}
                      className={cn(
                        "key rounded-xl py-2 text-xs font-bold flex items-center justify-center gap-1 cursor-pointer",
                        isSelected ? "text-cyan-hud font-extrabold" : "text-foreground hover:text-cyan-hud",
                      )}
                    >
                      <Volume2 className="w-3.5 h-3.5 text-cyan-400" />
                      <span>{isSelected ? "Current" : "Swap Voice"}</span>
                    </button>
                    <button
                      onClick={() => setDelegateOpenFor(a.id)}
                      className="key rounded-xl py-2 text-xs font-bold text-violet-hud flex items-center justify-center gap-1 cursor-pointer"
                    >
                      <Send className="w-3.5 h-3.5" />
                      <span>Delegate</span>
                    </button>
                  </div>
                )}

                <button
                  onClick={() => toggleAgent(a.id)}
                  className={cn(
                    "key w-full rounded-xl py-2 text-[11px] font-bold cursor-pointer",
                    a.status === "running" ? "text-amber-hud" : "text-emerald-hud",
                  )}
                >
                  {a.status === "running" ? "Suspend Agent" : "Activate Agent"}
                </button>
              </div>
            </article>
          );
        })}
      </div>
    </div>
  );
}
