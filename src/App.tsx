...
import { NluInsightModal } from './components/NluInsightModal';
import { MultiAgentStatusModal } from './components/MultiAgentStatusModal';
...
export default function App() {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'workspace_action') {
          ...
          // Hands-free voice vision control (Screen Sharing & Camera Activation)
          if (tn === 'start_screen_sharing' || (tn === 'control_vision_mode' && msg.args?.mode === 'screen' && msg.args?.action !== 'stop')) {
            startVision('screen');
          } else if (tn === 'start_camera_vision' || (tn === 'control_vision_mode' && msg.args?.mode === 'camera' && msg.args?.action !== 'stop')) {
            startVision('camera');
          } else if (tn === 'stop_screen_sharing' || tn === 'stop_camera_vision' || tn === 'stop_all_vision' || (tn === 'control_vision_mode' && msg.args?.action === 'stop')) {
            stopVision();
          }
  ...
  return (
    <div className="h-screen w-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans antialiased relative overflow-hidden">
      {/* Vibrant Background Floating Glowing Orbs (Glassmorphism Core characteristic) */}
      ...
      <div className="flex-1 min-h-0 w-full relative z-10 overflow-hidden">
        {/* Central Main Voice Workspace (Fixed Position Always Centered) */}
