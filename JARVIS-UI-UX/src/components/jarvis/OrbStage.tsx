...
/** Tick coordinates precomputed once with fixed precision so SSR and
 *  client produce byte-identical markup (no hydration drift from float repr). */
...
/** Hue shifts as core load rises — cyan → violet → amber → rose. */
...
/** Skeuomorphic core gauge cluster — machined dial with a glass dome over a live readout. */
export function OrbStage() {
  const { thinking, cpu, ram, net } = useJarvis();
  const stats = useStats();
  const active = thinking || stats.active > 0;
  const loadColor = loadHue(cpu);
  ...
  return (
    <div className="bezel relative grid min-h-0 flex-1 place-items-center overflow-hidden rounded-2xl p-3">
      ...
      <svg viewBox="0 0 760 520" className="relative h-full max-h-[24rem] w-full">
        ...
        {/* rainbow orbit — extra colour, slow drift */}
        <g style={{ transformOrigin: "380px 256px" }} className="animate-spin-slower">
          <circle
            cx="380"
            cy="256"
      ...
      <div className="neu-inset absolute bottom-3 flex items-center gap-3 rounded-full px-4 py-2">
        ...
        <span className="text-[11px] font-bold uppercase tracking-[0.22em] text-muted-foreground">
          {thinking ? "Processing" : stats.active > 0 ? "Operating" : "Standing by"}
        </span>
      </div>
    </div>
  );
